...
class SAGEApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Title on left — SAGE branding in purple accent
        self.header = ctk.CTkLabel(
            header_frame,
            text="SAGE  ·  PsYcGoD AI&ML",
        ...
        # Provider selector button — opens the dropdown on click
        self._ai_provider_btn = ctk.CTkButton(
            ai_frame,
            text=f"⚙ {self.config.get_default_ai().capitalize()}",
            command=self._open_provider_selector,
            width=110,
            height=28,
            fg_color="#7c3aed",
            hover_color="#6d28d9",
        ...
        self._ai_provider_btn.pack(side="left", padx=(0, 5))

        # Hidden combobox used to track value (still needed by existing logic)
        self.ai_selector = ctk.CTkComboBox(
            ai_frame,
            values=["Claude", "Codex", "Ollama", "Gemini", "Llama", "Mistral"],
            command=self._on_ai_combo_changed,
            state="readonly",
            width=0,
        )
        self.ai_selector.set(self.config.get_default_ai().capitalize())
        self.ai_selector.pack(side="left", padx=0)
        self.ai_selector.pack_forget()

        # Connect button — default blue theme
        ...
        # Right metrics panel: the 4 cards stacked vertically with breathing room.
        metrics_panel = ctk.CTkFrame(self, fg_color=("#f0f0f0", "#161616"), width=320)
        ...
        metrics_title.grid(row=0, column=0, padx=16, pady=(16, 8), sticky="ew")
    ...
    def copy_debug_bundle(self) -> None:
        ...
        try:
            with connect() as conn:
                ...
                compression = self._fetch_context_token_totals(conn)
    ...
    def _render_output_tabs(self) -> None:
        ...
        for tab_id, tab in sorted(self.output_tabs.items()):
            ...
            btn = ctk.CTkButton(
                ...
                hover_color="#1d4ed8" if active else "gray30",
                text_color="#ffffff" if active else "gray60",
            ...
            close_btn = ctk.CTkButton(
                ...
                width=28,
                height=28,
                fg_color="#2a2a2a" if active else "#1e1e1e",
                hover_color="#7f1d1d",
                text_color="#6b6b6b",
                font=ctk.CTkFont(size=12, weight="bold"),
            )
    ...
    def _activate_output_tab(self, tab_id: int) -> None:
        ...
        if self.active_output_tab_id in self.output_tabs:
            old_terminal = self.output_tabs[self.active_output_tab_id]["terminal"]
            old_terminal.grid_remove()

        tab = self.output_tabs[tab_id]
        self.active_output_tab_id = tab_id
        self.output_view = tab["terminal"]
        self.output_view.grid(row=0, column=0, sticky="nsew")
        self.output_stack.update_idletasks()
        ...
        self._ai_provider_btn.configure(text=f"⚙ {str(ai_name).capitalize()}")
        self.sidebar.set_provider(str(ai_name).capitalize())
        ...
        self._ai_provider_btn.configure(
            state="disabled" if self.ai_connected else "normal",
            fg_color="#4a4a4a" if self.ai_connected else "#7c3aed",
        )
        if self.ai_running:
    ...
    def create_metric_cards(self, parent):
        ...
        self.commands_card.grid(row=1, column=0, padx=16, pady=(8, 6), sticky="new")
        ...
        self.tokens_card.grid(row=2, column=0, padx=16, pady=6, sticky="new")
        ...
        self.agents_card.grid(row=3, column=0, padx=16, pady=6, sticky="new")
        ...
        self.success_card.grid(row=4, column=0, padx=16, pady=(6, 16), sticky="new")
    ...
    def _fetch_metrics(self):
        ...
        try:
            with connect() as conn:
                ...
                token_row = self._fetch_context_token_totals(conn)
                ...
                # Only count running/waiting — queued agents haven't started yet
                # so lighting them all green at enqueue time causes mass flicker.
                active_type_rows = conn.execute(
                    """
                    SELECT DISTINCT a.type
                    FROM agent_runs ar
                    JOIN agents a ON a.id = ar.agent_id
                    WHERE ar.status IN ('running', 'waiting_for_tool')
            ...
            self.after(0, lambda: self._update_ui_metrics(
                ...
                original_tokens, compressed_tokens, token_savings, token_rate, session_used, session_saved,
    ...
    def _update_ui_metrics(
        ...
        all_estimated_k = self._format_count(original_tokens)
        ...
        self.tokens_card.update_metric(
            ...
            session_hint=f"{session_token_rate:.0f}% saved",
            detail=f"{token_rate:.0f}% saved overall ({all_estimated_k} est.)",
        )

        self.agents_card.update_metric(
            total_value=f"{total_agents}",
            session_value=f"{session_agent_tasks}",
            total_hint=f"{running_agents} run, {queued_agents} q",
            session_hint=f"{waiting_agents} wait",
            detail=f"Run: {latest_agent_completed}/{latest_agent_total} done",
        )

        self.success_card.update_metric(
            total_value=f"{success_rate * 100:.0f}%",
            session_value=f"{session_success_rate * 100:.0f}%",
    ...
    def _fetch_context_token_totals(self, conn):
        """Return the same all-time token totals used by `sage context stats`."""
        table = conn.execute(
        ...
        row = conn.execute(
            """
            SELECT
                COALESCE(SUM(estimated_tokens), 0) as total_estimated,
    ...
    def _open_provider_selector(self):
        """Show a provider picker popup anchored to the provider button."""
        providers = ["Claude", "Codex", "Ollama", "Gemini", "Llama", "Mistral"]
        ...
        try:
            x = self._ai_provider_btn.winfo_rootx()
            y = self._ai_provider_btn.winfo_rooty() + self._ai_provider_btn.winfo_height()
            menu.tk_popup(x, y)
        except Exception:
            log.debug("suppressed", exc_info=True)

    def _select_provider(self, name: str):
        """Set the chosen provider from the popup menu."""
        self.ai_selector.set(name)
        self._ai_provider_btn.configure(text=f"⚙ {name}")
        self.on_ai_changed(name)

    def _on_ai_combo_changed(self, ai_name: str):
        """Bridge: if the hidden combo somehow fires, sync the button."""
        self._ai_provider_btn.configure(text=f"⚙ {ai_name}")
        self.on_ai_changed(ai_name)
    ...
    def on_connect_ai(self):
        ...
        if self.ai_connected:
            ...
            try:
                ...
                self._ai_provider_btn.configure(state="normal", fg_color="#7c3aed")
                self._save_active_output_tab_state()
                self._render_output_tabs()
    ...
    def _connect_selected_ai(self, show_status: bool = True) -> bool:
        ...
        try:
            ...
            if ai_name == "claude":
                ...
                self._ai_provider_btn.configure(state="disabled", fg_color="#4a4a4a")
            ...
            if not self.persistent_client.start_session():
                ...
                if show_status:
                    ...
                    self.output_view.append_text(
                        ...
                        f"- Codex: OPENAI_API_KEY env var or logged in via 'codex login'\n"
    ...
    def on_send_command(self, command: str):
        ...
        if not self.ai_connected or (not self.persistent_client and not self.current_client):
            connected = self._connect_selected_ai(show_status=False)
            if not connected and not getattr(self, "_api_travel_enabled", True):
                return False
        ...
        if (getattr(self, "_omniroute_enabled", False)
            ...
            try:
                ...
                LOG.info(
                    "API Travel: complexity=%s → agent=%s (%s)",
                ...
                if routed_agent != ai_name:
                    if routed_agent == "claude":
                        self.output_view.append_text(
                            f"\n[API Travel] {complexity} → {route_label}\n", "info"
                        )
                        ...
                        if travel_c is not None:
                            travel_c._api_travel_label = f"API Travel → {route_label}"
                            try:
                                if history:
                ...
                LOG.warning("API Travel classify/route failed, using selected agent: %s", e)
        ...
        if ai_name == "claude":
            ...
            result = self._run_persistent_client(contextual_command, ai_name, visible_prompt=command)
        ...
        try:
            ...
            for candidate in ("claude", "codex", "ollama"):
                if candidate == ai_name or candidate not in avail:
                    continue
                travel_c = self._get_travel_client(candidate)
                if travel_c is None:
                    continue
                travel_c._api_travel_label = f"Fallback → {candidate.capitalize()}"
                try:
                    msgs = self.session_manager.get_messages(
                        os.getcwd(), str(self.current_session_id or "")
                    )
                    ...
                    pass
                self.output_view.append_text(
                    f"\n[Fallback] {ai_name.capitalize()} unavailable — routing to {candidate.capitalize()}\n",
                    "info",
                )
                return self._run_persistent_client(
                    command, candidate, visible_prompt=command, _travel_client=travel_c
                )
        except Exception as e:
    ...
    def _send_prompt_to_terminal_ai(self, command: str) -> bool:
        ...
        try:
            ...
            # Inject conversation memory so the terminal AI knows prior context.
            # The CLI maintains its own session but a fresh connection has no history.
            contextual_prompt = self._build_contextual_prompt(visible_prompt)
            terminal.write(contextual_prompt + "\r\n")
    ...
    def _run_persistent_client(self, prompt: str, ai_name: str, visible_prompt: str | None = None, _travel_client: PersistentAIClient | None = None) -> bool:
        ...
        def flush_ui_buffer() -> None:
            ...
            if not ops:
                return
            # Batch all text inserts into ONE widget state change
            ...
            def flush_text() -> None:
                if pending_text:
                    ...
                    pending_text.clear()
                    output_view.text_widget.configure(state="normal")
                    output_view.text_widget.insert("end", combined)
                    output_view.text_widget.configure(state="disabled")
                    output_view._visible_chars += len(combined)
            ...
            for op in ops:
                if op[0] == "text":
                    ...
                    op[1]()
            flush_text()
            if output_view.auto_scroll:
                output_view._deferred_scroll()

        def _schedule_flush() -> None:
            if ui_flush_state["scheduled"]:
                return
            ui_flush_state["scheduled"] = True
            self.after(16, flush_ui_buffer)
        ...
        def stream_worker():
            ...
            try:
                ...
                # Collapsible sections: one growing section per event type per response.
                # Mutable holders so the closures can update the section id in-place.
                tool_sid:     list[str | None] = [None]
                thinking_sid: list[str | None] = [None]
                coding_sid:   list[str | None] = [None]
                ...
                def _queue_thinking_event(c: str) -> None:
                    def _run(c=c, view=output_view, holder=thinking_sid):
                        if holder[0] is None:
                            holder[0] = view.append_expandable_section(
                                "Thinking", c, "thinking_text", collapsed=False
                            )
                        else:
                            view.append_to_expandable_section(holder[0], c)
                    queue_ui_op(_run)

                def _queue_coding_event(c: str) -> None:
                    def _run(c=c, view=output_view, holder=coding_sid):
                        if holder[0] is None:
                            holder[0] = view.append_expandable_section(
                                "Coding", c, "code", collapsed=False
                            )
                        else:
                            view.append_to_expandable_section(holder[0], c)
                    queue_ui_op(_run)
                ...
                for event_type, content in client.send_message(prompt):
                    ...
                    if event_type == "thinking":
                        ...
                        _queue_thinking_event(content)
                    elif event_type == "coding":
                        assistant_chunks.append(content)
                        _queue_coding_event(content)
                        ...
                        # Post-response footer — agent used, token stats, compression %
                        route_label = getattr(client, "_api_travel_label", None) or ai_name.capitalize()

                        def _append_response_footer(rlbl=route_label, rid=run_id):
                            orig = 0
                            saved = 0
                            try:
                                from sage.store import connect as _store_connect
                                with _store_connect() as _conn:
                                    row = _conn.execute(
                                        "SELECT original_tokens, tokens_saved FROM context_compression WHERE run_id = ?",
                                        (rid,)
                                ...
                                pass
                            pct = int(saved * 100 / max(orig, 1)) if orig else 0
                            parts = [f"via {rlbl}"]
                            if orig:
                                parts.append(f"{orig:,} tokens")
                            if saved:
                                parts.append(f"saved {saved:,} ({pct}%)")
                            footer = "  ·  ".join(parts)
                            output_view.append_text(f"\n{'─' * 56}\n{footer}\n", "info")

                        queue_ui_op(_append_response_footer)

                        # EXECUTE AGENTS NOW — reuse the run_id from the saved AI run
                        threading.Thread(
                            target=self._execute_agents_for_response,
                            args=(prompt, visible_prompt, run_id, output_view),
                            daemon=True,
                        ).start()
    ...
    def _external_terminal_command(self, ai_name: str, human_readable: bool = True) -> str:
        ...
        # Only borrow current_client.command when it belongs to the same agent.
        # If ai_name is "codex" but current_client is a Claude CLIClient, using
        # its command would run Claude in the terminal instead of Codex.
        if (self.current_client
                and getattr(self.current_client, "command", "")
                and getattr(self.current_client, "ai_name", None) == ai_name):
    ...
    def _execute_agents_for_response(
        self,
        command: str,
        visible_command: str,
        ...
        try:
            ...
            if results:
                ...
                self.after(
                    0,
                    lambda r=results, v=view, rid=run_id, ms=duration_ms: self._display_agent_results(
                        r, v, rid, ms
                    ),
                )
    ...
    def _display_agent_results(
        ...
        """Display agent analysis results in the active tab's output view."""
        view = output_view or self.output_view
        if not view or not results:
            return

        try:
            agent_names = []
            for result in results:
                label = self._agent_display_label(result)
                if label not in agent_names:
                    agent_names.append(label)

            used_tokens, saved_tokens = self._agent_analysis_token_totals(run_id)
            ml_line = self._agent_analysis_ml_line(results)
            error_line = self._agent_analysis_error_line(results)

            analysis_body = f"Agents - {', '.join(agent_names)}\n"
            if error_line:
                analysis_body += f"Result - {error_line}\n"
            analysis_body += f"Token - Used {used_tokens:,} / Saved {saved_tokens:,}\n"
            if ml_line:
                analysis_body += f"ML - {ml_line}\n"
            analysis_body += f"Time - {self._format_agent_duration(duration_ms)}\n"
            view.append_expandable_section(
                "Agent Analysis", analysis_body, "info", collapsed=True
            )

        except Exception as e:
            print(f"[SAGE] Failed to display agent results: {e}")

    def _agent_display_label(self, result: dict) -> str:
        ...
        if agent_type in labels:
            return labels[agent_type]
        agent_name = str(result.get("agent") or result.get("agent_name") or "Agent").strip()
        return agent_name.replace(" Agent", "").strip() or "Agent"

    def _agent_analysis_token_totals(self, run_id: int | None) -> tuple[int, int]:
        if not run_id:
            return 0, 0
        try:
            with connect() as conn:
                row = conn.execute(
                    """
                    SELECT compressed_tokens, saved_tokens
                    FROM context_compression
                    WHERE run_id = ?
            ...
            LOG.debug("Could not load agent analysis token totals", exc_info=True)
        return 0, 0

    def _agent_analysis_ml_line(self, results: list[dict]) -> str:
        ml_result = next((item for item in results if str(item.get("agent_type") or "").lower() == "ml"), None)
        if ml_result:
            conf = float(ml_result.get("confidence") or 0.0)
            if conf < 0.75:
                return ""
            return str(ml_result.get("finding") or "ML analysis complete").strip()

        highest = max((float(item.get("confidence") or 0.0) for item in results), default=0.0)
        if highest < 0.75:
            return ""
        if any(str(item.get("severity") or "").lower() in {"high", "medium"} for item in results):
            return f"Risk detected ({highest:.0%} confidence)"
        return f"No risk ({highest:.0%} confidence)"

    def _agent_analysis_error_line(self, results: list[dict]) -> str:
        caught = []
        for result in results:
            ...
            has_error = (
                ...
                or bool(signals.get("has_error"))
                or severity in {"high", "medium"}
            )
            if not has_error:
                continue
            label = self._agent_display_label(result)
            finding = str(result.get("finding") or "error signal found").strip()
            caught.append(f"{label} caught: {finding}")
        return "; ".join(caught[:3])

    def _format_agent_duration(self, duration_ms: int | None) -> str:
    ...
    def _scan_skill_files(self) -> list[dict]:
        ...
        try:
            from sage.skills import SKILLS_DIR

            roots.insert(0, (SKILLS_DIR, "SAGE"))
        except Exception:
            pass
    ...
    def load_sidebar_data(self):
        ...
        self._refresh_sidebar_providers()

    def _refresh_sidebar_providers(self):
        """Update sidebar AI provider list based on what's available."""
        try:
            base = ["Claude", "Codex", "Ollama", "Gemini"]
            if getattr(self, "_api_travel_enabled", True):
                available = api_travel.detect_available()
                if len(available) > 1:
                    base = ["API Travel"] + base
            self.sidebar.set_provider_options(base)
        except Exception:
            log.debug("suppressed", exc_info=True)
    ...
    def reset_dashboard_data(self) -> bool:
        ...
        try:
            ...
            self._update_ui_metrics(
                ...
                totals.get("total_agent_tasks", 0),
                0,
                success_rate,
                0,
                successful_commands,
    ...
    def handle_chat_action(self, action: str, chat: dict):
        ...
        if action == "switch_provider":
            provider = chat.get("provider", "Claude")
