...
class SettingsPanel(ctk.CTkToplevel):
    ...
    def _create_api_keys_section(self, parent):
        """Auto-detected CLI agents status (read-only)."""
        import shutil
        section = self._create_section(parent, "Connected AI Agents (auto-detected)", row=3)
        ...
        for i, (label, binary) in enumerate(agents):
            found = shutil.which(binary) is not None
            icon = "●" if found else "○"
            color = "#4ade80" if found else "gray50"
            ...
            ctk.CTkLabel(
                ...
                text_color=color,
                anchor="w",
            ).grid(row=i, column=0, columnspan=2, padx=15, pady=2, sticky="w")

        note = ctk.CTkLabel(
            section,
            text="SAGE auto-uses these CLIs. Manual API keys below are fallback only.",
            font=ctk.CTkFont(size=10),
            text_color="gray60"
        )
        note.grid(row=len(agents), column=0, columnspan=2, padx=15, pady=(6, 10), sticky="w")
    ...
    def _create_api_travel_section(self, parent):
        ...
        self._api_travel_switch = ctk.CTkSwitch(
            section,
            text="Enable API Travel  (auto-route to cheapest capable agent)",
            font=ctk.CTkFont(size=12),
    ...
    def _on_api_travel_toggle(self):
        try:
            enabled = bool(self._api_travel_switch.get())
            self.parent_app._api_travel_enabled = enabled
            self.parent_app._omniroute_enabled = enabled
        except Exception:
            pass

    def _on_omniroute_toggle(self):
        self._on_api_travel_toggle()
