...
class FloatingSidebar(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── Top-left: SAGE branding + profile ──
        header_frame = ctk.CTkFrame(self, fg_color="transparent")
        header_frame.grid(row=0, column=0, padx=15, pady=(15, 6), sticky="ew")
        ...
        tagline_label = ctk.CTkLabel(
            header_frame,
            text="By PsYcGoD AI&ML",
            font=ctk.CTkFont(size=11),
            text_color="#a78bfa",
            anchor="w",
        )
        tagline_label.grid(row=1, column=0, sticky="w", pady=(0, 4))
        ...
        settings_btn.grid(row=0, column=1, rowspan=2, sticky="ne", padx=(0, 0))
        ...
        # ── Bottom-left: AI provider selector ──
        bottom_frame = ctk.CTkFrame(self, fg_color="transparent")
        bottom_frame.grid(row=4, column=0, padx=15, pady=(8, 15), sticky="ew")
        bottom_frame.grid_columnconfigure(0, weight=1)

        ai_provider_label = ctk.CTkLabel(
            bottom_frame,
            text="AI Provider",
        ...
        ai_provider_label.grid(row=0, column=0, sticky="w", pady=(0, 2))

        self.ai_provider_combo = ctk.CTkComboBox(
            bottom_frame,
            values=["Claude", "Codex", "Ollama", "Gemini"],
            state="readonly",
            height=32,
            font=ctk.CTkFont(size=12),
            command=self._on_provider_changed,
        )
        self.ai_provider_combo.set("Claude")
        self.ai_provider_combo.grid(row=1, column=0, sticky="ew")

        self.load_project_groups([])

    def _on_provider_changed(self, choice: str):
        """Notify the parent app that the user picked a new AI provider."""
        if self.on_chat_action:
            self.on_chat_action("switch_provider", {"provider": choice})

    def set_provider(self, name: str):
        """Set the active provider shown in the bottom combo."""
        try:
            self.ai_provider_combo.set(name)
        except Exception:
            pass

    def set_provider_options(self, options: list[str]):
        """Update available providers (e.g. only connected ones)."""
        try:
            self.ai_provider_combo.configure(values=options)
        except Exception:
            pass
