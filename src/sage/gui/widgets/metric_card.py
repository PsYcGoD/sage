...
class DualMetricCard(ctk.CTkFrame):
    ...
    def __init__(self, master, label: str, **kwargs):
        ...
        label_font_size = int(kwargs.pop("label_font_size", 12))
        title_font_size = int(kwargs.pop("title_font_size", 12))
        value_font_size = int(kwargs.pop("value_font_size", 18))
        muted_font_size = int(kwargs.pop("muted_font_size", 10))
        kwargs["height"] = max(int(kwargs.get("height", 140)), 120)
        super().__init__(
            master,
            corner_radius=10,
            fg_color=("#E8E8E8", "#1e1e2e"),
            **kwargs
        )
        ...
        self.label_widget.grid(row=0, column=0, columnspan=2, padx=12, pady=(10, 4), sticky="ew")

        self.total_title = self._make_label("Total", title_font_size, "normal", "w")
        self.total_title.grid(row=1, column=0, padx=(12, 4), sticky="w")

        self.session_title = self._make_label("Session", title_font_size, "normal", "e")
        self.session_title.grid(row=1, column=1, padx=(4, 12), sticky="e")

        self.total_value = self._make_label("0", value_font_size, "bold", "w")
        self.total_value.grid(row=2, column=0, padx=(12, 4), pady=(2, 0), sticky="w")

        self.session_value = self._make_label("0", value_font_size, "bold", "e")
        self.session_value.grid(row=2, column=1, padx=(4, 12), pady=(2, 0), sticky="e")

        self.total_hint = self._make_muted_label("", "w")
        self.total_hint.grid(row=3, column=0, padx=(12, 4), pady=(2, 0), sticky="w")

        self.session_hint = self._make_muted_label("", "e")
        self.session_hint.grid(row=3, column=1, padx=(4, 12), pady=(2, 0), sticky="e")

        self.detail = self._make_muted_label("", "w")
        self.detail.grid(row=4, column=0, columnspan=2, padx=12, pady=(4, 10), sticky="ew")
