...
class OutputView(ctk.CTkFrame):
    ...
    def _deferred_scroll(self):
        """Scroll to bottom with deduplication — avoids hammering scroll on fast streaming."""
        if not getattr(self, "_scroll_pending", False):
            self._scroll_pending = True
            self.after(50, self._do_deferred_scroll)
    ...
    def begin_ai_stream(self, ai_name: str, route_label: str | None = None):
        """Start a left-aligned assistant response, with optional API-Travel badge."""
    ...
    def append_expandable_section(
        ...
        *,
        ...
        if collapsed:
            # Use underlying tk.Text for elide — CTkTextbox may not pass it through
            raw_text = self.text_widget._textbox if hasattr(self.text_widget, '_textbox') else self.text_widget
            raw_text.tag_configure(body_tag, elide=True)
    ...
    def toggle_expandable_section(self, section_id: str) -> None:
        ...
        raw_text = self.text_widget._textbox if hasattr(self.text_widget, '_textbox') else self.text_widget
        raw_text.tag_configure(str(section["body_tag"]), elide=collapsed)
        self.text_widget.configure(state="disabled")
